...
class WhatsAppController extends Controller
{
    ...
    private function startContPaqiFlow($chat, $from, $text)
    {
        ...
        if (str_contains($text, 'contab')) {
            ...
            $this->updateChatQuestion($chat, 'contpaqi_detalle_comercial');
            ...
            $this->updateChatQuestion($chat, 'contpaqi_detalle_bancos');
    ...
    private function handleNubeFlow($chat, $from, $text)
    {
        ...
        if ($lastQuestion === 'nube_software') {
            ...
            if (str_contains($text, '1') || str_contains($text, 'contpaqi')) {
                $software = 'CONTPAQi';
            } elseif (str_contains($text, '2') || str_contains($text, 'office')) {
                $software = 'Office 365';
            } elseif (str_contains($text, '3') || str_contains($text, 'otros')) {
                $software = 'Sistemas personalizados';
            } elseif (str_contains($text, '4') || str_contains($text, 'todos')) {
    ...
    private function handleCapacitacionFlow($chat, $from, $text)
    {
        ...
        if ($lastQuestion === 'capacitacion_modalidad') {
            $modalidad = '';
            if (str_contains($text, '1') || str_contains($text, 'presencial')) {
                $modalidad = 'Presencial';
            } elseif (str_contains($text, '2') || str_contains($text, 'virtual')) {
                $modalidad = 'Virtual';
            } else {
                $modalidad = 'Híbrida';
            }
    ...
    // ============================================
    // MÉTODOS HELPER PARA ACTUALIZACIÓN GARANTIZADA
    // ============================================
    ...
    private function updateChatContext($chat, $context)
    {
        ...
        $chat->save();
        $chat->refresh();
        \Log::info("✅ Context actualizado a: {$context}", ['chat_id' => $chat->id]);
    }
    ...
    private function updateChatQuestion($chat, $question)
    {
        ...
        $chat->save();
        $chat->refresh();
        \Log::info("✅ Last_bot_question actualizado a: {$question}", ['chat_id' => $chat->id]);
    }
    ...
    private function updateChatMetadata($chat, array $metadata)
    {
        ...
        $chat->save();
        $chat->refresh();
        \Log::info('✅ Metadata actualizado', ['chat_id' => $chat->id, 'metadata' => $metadata]);
    }

    private function updateChatStatus($chat, $status)
    {
        $chat->status = $status;
        $chat->save();
        $chat->refresh();
        \Log::info("✅ Status actualizado a: {$status}", ['chat_id' => $chat->id]);
    }
    ...
    private function updateChatFull($chat, $data)
    {
        ...
        $chat->save();
        $chat->refresh();
        \Log::info('✅ Chat actualizado completamente', ['chat_id' => $chat->id, 'data' => $data]);
    }
    ...
    private function updateConversationHistory($chat, $message, $sender)
    {
        ...
        $chat->save();
        $chat->refresh();
        \Log::info('✅ Conversation history actualizado', ['chat_id' => $chat->id]);
    }
